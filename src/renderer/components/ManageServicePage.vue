  ...
  <b-container fluid id="wrapper">
    ...
    <b-row class="body">
      ...
      <template v-slot:row-details="row">
        <b-card>
          <b-row class="mb-2">
            <b-col sm="3" class="text-sm-right"><b>TÊN DỊCH VỤ</b></b-col>
            <b-col>
              <b-form-input
          ...
          <b-row class="mb-2">
            <b-col sm="3" class="text-sm-right"><b>NHÓM DỊCH VỤ</b></b-col>
            <b-col>
              <b-form-input
              :value="row.item.groupDisplayName"
              @change="(e) => setFormData(row.item.name, 'groupDisplayName', e)"
              type="text"
              required>
              </b-form-input></b-col>
          </b-row>
          <b-row class="mb-2">
            <b-col sm="3" class="text-sm-right"><b>GIÁ TIỀN</b></b-col>
            <b-col>
              <b-form-input
              :value="row.item.cost"
              @change="(e) => setFormData(row.item.name, 'cost', !e ? 0 : parseInt(e))"
              type="number"
              required>
              </b-form-input></b-col>
          </b-row>
         <b-row class="mb-2">
            <b-col sm="3" class="text-sm-right"><b>THỨ TỰ</b></b-col>
            <b-col>
              <b-form-input
              :value="row.item.sort"
          ...
          <b-row class="mb-2">
            <b-col sm="3" class="text-sm-right"><b>HÌNH ẢNH</b></b-col>
            <b-col>
              <b-form-input
              :value="row.item.imagePath"
              placeholder="Bạn chưa chọn hình đại diện"
              @change="(e) => setFormData(row.item.name, 'imagePath', e)"
              type="text"
              required>
              </b-form-input>
              <img v-bind:src="row.item.imagePath || noImageUriData" class="preview-image"/>
              </b-col>
          </b-row>
          <b-row class="mb-2">
             <b-col sm="3" class="text-sm-right"><b></b></b-col>
            <b-col>
              <b>{{row.item.inactive ? 'ĐANG TẠM DỪNG BÁN DỊCH VỤ': ''}}</b>
              <br/>
    ...
    <b-button variant="outline-primary" @click="addNewService()" class="mr-2">
          + THÊM DỊCH VỤ
        </b-button>
      </b-container>
    </b-row>
...
export default {
  ...
  methods: {
    ...
    addNewService () {
      this.$store.commit('Admin/ADD_EMPTY_SERVICE')
    },
    cancel () {
      this.$router.push({ name: 'landing-page' })
    },
    setFormData (serviceName, propName, propValue) {
      this.$store.commit('Admin/UPDATE_SERVICE', { serviceName, propName, propValue })
    },
    saveServiceJson () {
      this.$store.dispatch('Admin/saveServiceJson')
    }
  },
  computed: {
    ...mapState({
